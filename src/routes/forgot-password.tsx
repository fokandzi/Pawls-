...
export const Route = createFileRoute("/forgot-password")({
  head: () => seoHead(SEO["forgot-password"]),
      ...
      <section className="bg-gradient-to-b from-[var(--pawls-cream-50)] to-white px-6 pb-20 pt-16 sm:pt-20">
        <div className="mx-auto w-full max-w-md">
          <div className="text-center">
            <h1 className="mt-4 text-3xl font-extrabold tracking-tight text-gray-900">Reset your password</h1>
            <p className="mt-3 text-base text-gray-600">Enter your email and we'll send you a reset link (valid for 1 hour).</p>
          </div>
          <form action="/auth/password-reset-request" method="POST" className="mt-10 space-y-5 rounded-2xl border border-[var(--pawls-cream-200)] bg-white p-6 shadow-lg sm:p-8">
            ...
            <button type="submit" className="inline-flex w-full items-center justify-center rounded-full bg-[var(--pawls-terracotta-500)] px-6 py-3.5 text-base font-semibold text-white shadow-lg shadow-[var(--pawls-terracotta-500)]/25 transition-colors hover:bg-[var(--pawls-terracotta-700)]">Send reset link</button>
            <p className="text-center text-sm text-gray-500">
              <Link to="/login" className="font-medium text-[var(--pawls-terracotta-500)] underline hover:text-[var(--pawls-terracotta-700)]">← Back to login</Link>
            </p>
          </form>
        </div>
      </section>
